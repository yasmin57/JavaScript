//Json com estados e suas cidades
...
//Variaveis com as ids dos elementos com html
...
//filtra a sigla
const filtraSigla = (estadosCidades) => estadosCidades.sigla == $sigla.value;

// filtra o json para o estado desejado usando a sigla digitada
const pegaEstado = (json) => json.filter(filtraSigla);

// Cria a estrutura em html
const mandaDados = (acumuladora, json, distribuiIds, corTd) => `${acumuladora} <tr ${corTd} ><td>${distribuiIds} - ${json.nome}</td></tr>` ;

// Reduz o json apenas para as cidades e chama a função mandaDados
const pegaCidades = (json) => json[0].cidades.reduce(mandaDados, ""); 

//Distribui ids
const distribuiIds = () =>  i++;

//Seleta um cor usando a id como referência
const corTd = (distribuiIds) => {
   if(distribuiIds%2 == 0){
      let style = 'style="background-color: #2d3238; color:white;"'
      return style;
   }
   else{
      return 'style="background-color: #808891; color:black;"'
   }
} 

// No click do botão pesquisar faz a chamada da função pegaCidades 
$pesquisar.addEventListener("click", () => $resultado.innerHTML = pegaCidades(pegaEstado(estadosCidades)));
